                ...
                <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="relative max-w-4xl mx-auto">
                        ...
                        {phases.map((phase, index) => (
                            <ScrollReveal key={phase.number} animation={index % 2 === 0 ? 'fadeLeft' : 'fadeRight'} delay={0.2}>
                                <div className={`relative mb-20 last:mb-0 sm:pl-24 lg:pl-0 ${index % 2 === 0 ? 'lg:pr-[55%]' : 'lg:pl-[55%]'}`}>
                                    <motion.div whileHover={{ scale: 1.1 }} className={`absolute left-0 lg:left-1/2 lg:-translate-x-1/2 w-16 h-16 rounded-2xl bg-gradient-to-br ${phase.color} flex items-center justify-center text-white font-black text-xl hidden sm:flex glow-primary`}>
                                        {phase.number}
                                    </motion.div>
                                    <Card className={`border-zinc-800 hover:border-primary/30 ${index % 2 === 0 ? 'lg:mr-8' : 'lg:ml-8'}`}>
                                        <CardContent className="p-8">
                                            <div className={`flex items-start gap-4 mb-6 ${index % 2 === 0 ? 'lg:flex-row-reverse lg:text-right' : ''}`}>
                                                ...
                                                <div>
                                                    <h3 className="text-2xl font-black mb-2">Phase {phase.number}: {phase.title}</h3>
                                            ...
                                            <div className={`mb-6 ${index % 2 === 0 ? 'lg:text-right' : ''}`}>
                                                ...
                                                <StaggerList staggerDelay={0.08}>
                                                    {phase.deliverables.map((item) => (
                                                        <div key={item} className={`flex items-center gap-3 text-sm text-gray-400 mb-2 ${index % 2 === 0 ? 'lg:flex-row-reverse' : ''}`}>
                                            ...
                                            {phase.codeExample && (
                                                <motion.div className="rounded-xl bg-black/50 border border-zinc-800 p-4 overflow-x-auto" whileHover={{ scale: 1.01 }} transition={{ duration: 0.2 }}>
                                                    <pre className="text-xs font-mono text-gray-400"><code>{phase.codeExample}</code></pre>
                                                </motion.div>
                                            )}
                                        </CardContent>
                                    </Card>
